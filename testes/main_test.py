from src.main import _calc_meses, calc_valor_futuro, calc_valor_presente
...
def teste_calc_meses():
    ...
    esperado = 12
    resultado = _calc_meses("2024-08-30")
...
def teste_calc_valor_presente():
    ...
    qtd_meses = _calc_meses(data_futura_aleatorio)

    # Variáveis de teste
    resultado = calc_valor_presente(valor_futuro_aleatorio, 0.5, data_futura_aleatorio)
    esperado = valor_futuro_aleatorio / (1 + 0.5) ** qtd_meses

    # Deve retornar True
    assert round(resultado) == round(esperado)
...
def teste_calc_valor_futuro():
    ...
    # Variáveis de teste
    resultado = calc_valor_futuro(valor_presente_aleatorio, 0.5, data_futura_aleatorio)
